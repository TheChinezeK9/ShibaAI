...
export async function generateStudy(req, res) {
  const { tool, notes, difficulty = "standard", detail = "quick" } = req.body;
  ...
  try {
    ...
    const safeDetail = ["quick", "detailed"].includes(detail) ? detail : "quick";
    return res.json(await generateStudyMaterial(tool, notes.trim(), { difficulty: safeDifficulty, detail: safeDetail }));
  } catch (error) {
