...
import { AboutPage, ValuesPage, PricingPage, ResourcesPage, FAQPage, ContactPage } from "./components/MarketingPages.jsx";
...
function Header({ user, onHome, onOpenAuth, onDashboard, onLogout, onProfile, onNavigate, onSearch, theme, onToggleTheme }) {
  ...
  return (
    <header className="site-header">
      ...
      <nav>
        ...
        <div className={`nav-dropdown ${openMenu === "tools" ? "open" : ""}`} onMouseLeave={() => setOpenMenu(null)}>
          ...
          <div className="tools-dropdown">
            ...
            <div>{tools.map((tool) => <button key={tool.id} onClick={() => closeThen(() => onDashboard(tool.id))}><span>{tool.icon}</span><b>{tool.name}</b></button>)}</div>
          </div>
        </div>
      ...
      {mobileOpen && <div className="mobile-menu"><button onClick={() => mobileNavigate("home")}>Home</button><button onClick={() => { setMobileOpen(false); onDashboard(); }}>Study tools</button><button onClick={() => mobileNavigate("subjects")}>Subject guides</button><button onClick={() => mobileNavigate("about")}>About</button><button onClick={() => mobileNavigate("values")}>Our values</button><button onClick={() => mobileNavigate("pricing")}>Pricing</button><button onClick={() => mobileNavigate("resources")}>Resources</button><button onClick={() => mobileNavigate("faq")}>FAQ</button><button onClick={() => mobileNavigate("contact")}>Contact</button></div>}
    </header>
  );
}
...
function Dashboard({ initialTool, initialNotes = "", user }) {
  ...
  return <main className="dashboard-shell"><aside className="sidebar"><div><small>STUDY TOOLS</small>{tools.map((tool) => <button key={tool.id} className={activeId === tool.id ? "active" : ""} onClick={() => chooseTool(tool.id)}><span>{tool.icon}</span>{tool.name}</button>)}</div><div><FocusTimer /><div className="sidebar-tip"><span className="text-paw">🐾︎</span><strong>Shiba study tip</strong><p>Active recall beats rereading. Test yourself before reviewing.</p></div></div></aside><section className="workspace"><div className="workspace-welcome"><div><span className="eyebrow">GOOD TO SEE YOU, {user.name.toUpperCase()}</span><h1>{active.name}</h1><p>{active.description}</p></div><div className="streak"><span>🔥</span><div><small>STUDY STREAK</small><strong>1 day</strong></div></div></div><div className="generator-card"><div className="generator-title"><div><span className="paw-dot text-paw">🐾︎</span><label htmlFor="study-notes">Paste your notes or topic</label></div><span>Choose your level, then generate</span></div><textarea id="study-notes" value={notes} maxLength={12000} disabled={loading} onChange={(e) => setNotes(e.target.value)} placeholder="Paste class notes, a textbook passage, or describe the topic you want to study…" /><div className="generation-options"><label>Level<select value={difficulty} onChange={(event) => setDifficulty(event.target.value)} disabled={loading}><option value="simple">Simple</option><option value="standard">Standard</option><option value="advanced">Advanced</option></select></label>{activeId === "quiz" ? <label>Questions<input type="number" min="1" max="50" value={questionCount} onChange={(event) => setQuestionCount(Math.max(1,Math.min(50,Number(event.target.value) || 1)))} disabled={loading} aria-label="Number of quiz questions"/></label> : <label>Detail<select value={detail} onChange={(event) => setDetail(event.target.value)} disabled={loading}><option value="quick">Quick</option><option value="detailed">Detailed</option></select></label>}</div><div className="generator-actions"><span>{notes.length.toLocaleString()} / 12,000</span><button className="button" disabled={loading} onClick={generate}>{loading ? "Creating…" : `Generate ${active.name}`} <b>✦</b></button></div></div><ErrorMessage message={error} />{activeId === "quiz" ? <QuizResults questions={result?.questions || []} /> : <ToolResult result={result} />}{history.length > 0 && !result && <section className="recent-section"><div className="recent-heading"><div><span className="eyebrow">YOUR LIBRARY</span><h2>Recent study sessions</h2></div><button onClick={() => { setHistory([]); localStorage.removeItem("shibaai-history"); }}>Clear</button></div><div className="recent-grid">{history.map((item) => <button key={item.id} onClick={() => chooseTool(tools.find((tool) => tool.name === item.tool)?.id || "quiz")}><span>{item.icon}</span><div><strong>{item.title}</strong><small>{item.tool} · {item.date}</small></div><b>→</b></button>)}</div></section>}</section></main>;
}

const paths = { home:"/", about:"/about", values:"/values", pricing:"/pricing", subjects:"/subjects", resources:"/resources", faq:"/faq", contact:"/contact", dashboard:"/app" };
